...
export const reorderJob = async (id: string, data: { fromOrder: number; toOrder: number }) => {
  ...
  const updatedJobs = allJobs.map(job => {
    if (job.id === id) {
      return { ...job, order: data.toOrder };
    } else if (data.fromOrder < data.toOrder) {
      // Moving down: Shift jobs up that are between fromOrder and toOrder
      if (job.order > data.fromOrder && job.order <= data.toOrder) {
        return { ...job, order: job.order - 1 };
      }
    } else if (data.fromOrder > data.toOrder) {
      // Moving up: Shift jobs down that are between toOrder and fromOrder
      if (job.order >= data.toOrder && job.order < data.fromOrder) {
        return { ...job, order: job.order + 1 };
      }
    }
    return job;
  ...
  await jobsDb.transaction('rw', jobsDb.jobs, async () => {
    for (const job of updatedJobs) {
      await jobsDb.jobs.update(job.id, { order: job.order });
    }
  });
